...
FRAMEDB_USING_CONDA = os.environ.get("FRAMEDB_USING_CONDA", "0")
FRAMEDB_USING_CONDA = FRAMEDB_USING_CONDA != "0"
print(f"FRAMEDB_USING_CONDA={FRAMEDB_USING_CONDA}")

# flag to indicate if we want to build the tests
FRAMEDB_BUILD_CPP_TESTS = os.environ.get("FRAMEDB_BUILD_CPP_TESTS", "0")
FRAMEDB_BUILD_CPP_TESTS = FRAMEDB_BUILD_CPP_TESTS != "0"
print(f"FRAMEDB_BUILD_CPP_TESTS={FRAMEDB_BUILD_CPP_TESTS}")
...
class CompileProto(Command):
    # When adding new protobuf versions, also update: setup.cfg, python/framedb/__init__.py
    _PROTOBUF_TO_GRPC_VERSION = {"3": "<=1.30.*", "4": ">=1.49"}

    description = '"protoc" generate code _pb2.py from .proto files'
    user_options = [
        ("build-lib=", "b", "Create the framedb/proto/*/framecc/pb2 subdirectories inside this base directory"),
    ...
    def finalize_options(self):
        ...
        if self.proto_vers is None:
            self.proto_vers = os.getenv("FRAMEDB_PROTOC_VERS", "".join(self._PROTOBUF_TO_GRPC_VERSION)).strip()

    def run(self):
        output_dir = os.path.join(self.build_lib, "framedb", "proto")
        python = sys.version_info[:2]
        print(f"\nProtoc compilation (into '{output_dir}') for versions '{self.proto_vers}':")
        for proto_ver in self.proto_vers:
            if not FRAMEDB_USING_CONDA and python >= (3, 11) and proto_ver == "3":
            ...
            elif not FRAMEDB_USING_CONDA and python <= (3, 6) and proto_ver >= "4":
                # No available on PyPI for this configuration
    ...
    def _compile_one_version(self, proto_ver: str, version_output_dir: str):
        ...
        _log_and_run(*cmd, *glob.glob(os.path.normpath("cpp/proto/framecc/pb2/*.proto")), env=env)
...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if preset == "skip":
            return
        if preset == "*":
            conda_suffix = "-conda" if FRAMEDB_USING_CONDA else ""
        ...
        cmd = [
            cmake,
            f"-DTEST={FRAMEDB_BUILD_CPP_TESTS}",
            f"-DBUILD_PYTHON_VERSION={sys.version_info[0]}.{sys.version_info[1]}",
            f"-DCMAKE_INSTALL_PREFIX={os.path.dirname(dest)}",
            "--preset",
            preset,
        ]
        vcpkg_installed_dir = os.getenv("FRAMEDB_VCPKG_INSTALLED_DIR")
        print(f"FRAMEDB_VCPKG_INSTALLED_DIR={vcpkg_installed_dir}")
...
if __name__ == "__main__":
    setup(
        ext_modules=[CMakeExtension("framedb_ext")],
        package_dir={"": "python"},
        packages=find_packages(where="python", exclude=["tests", "tests.*"])
        + find_namespace_packages(where="python", include=["framedb.proto.*"]),
