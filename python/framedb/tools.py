...
from framedb.config import set_log_level, Defaults

from framedb_ext import set_config_int, set_config_string, set_config_double

# Setting config from environment variables. This code is used in the package __init__.py
_FRAMEDB_ENV_VAR_PREFIX = "FRAMEDB_"
_TYPE_INT = "INT"
_TYPE_FLOAT = "FLOAT"
_TYPE_STR = "STR"
...
def set_config_from_env_vars(env_vars: Dict[str, str]):
    ...
    The key names can be anything. Only those starting with the "FRAMEDB_" prefix are considered relevant.
    ...
    The framecc set_config_xxx() function corresponding to the type is called.
    """
    if env_vars is None:
        return
    ...
    for k, v in env_vars.items():
        k = k.upper()
        start_index = None
        if k.startswith(_FRAMEDB_ENV_VAR_PREFIX):  # 1 underscore in prefix
            start_index = 1
