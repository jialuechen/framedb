...
from framecc.pb2.descriptors_pb2 import IndexDescriptor, TypeDescriptor, SortedValue
from framecc.pb2.storage_pb2 import LibraryConfig, EnvironmentConfigsMap
from framedb.preconditions import check
from framedb.supported_types import DateRangeInput, ExplicitlySupportedDates
from framedb.toolbox.library_tool import LibraryTool
from framedb.version_store.processing import QueryBuilder
from framedb_ext.storage import OpenMode as _OpenMode
from framedb.encoding_version import EncodingVersion
from framedb_ext.storage import (
    create_mem_config_resolver as _create_mem_config_resolver,
    LibraryIndex as _LibraryIndex,
    Library as _Library,
)
from framedb.version_store.read_result import ReadResult
from framedb_ext.version_store import IndexRange as _IndexRange
from framedb_ext.version_store import RowRange as _RowRange
from framedb_ext.version_store import SignedRowRange as _SignedRowRange
from framedb_ext.version_store import PythonVersionStore as _PythonVersionStore
from framedb_ext.version_store import PythonVersionStoreReadQuery as _PythonVersionStoreReadQuery
from framedb_ext.version_store import PythonVersionStoreUpdateQuery as _PythonVersionStoreUpdateQuery
from framedb_ext.version_store import PythonVersionStoreReadOptions as _PythonVersionStoreReadOptions
from framedb_ext.version_store import PythonVersionStoreVersionQuery as _PythonVersionStoreVersionQuery
from framedb_ext.version_store import ColumnStats as _ColumnStats
from framedb_ext.version_store import StreamDescriptorMismatch
from framedb_ext.version_store import DataError
from framedb.authorization.permissions import OpenMode
from framedb.exceptions import ArcticDbNotYetImplemented, ArcticNativeException
from framedb.flattener import Flattener
from framedb.log import version as log
from framedb.version_store._custom_normalizers import get_custom_normalizer, CompositeCustomNormalizer
from framedb.version_store._normalization import (
    NPDDataFrame,
    normalize_metadata,
...
class NativeVersionStore:
    """
    NativeVersionStore objects provide access to FrameDB libraries, enabling fundamental library operations
    ...
    def create_library_config(cfg, env, lib_name, encoding_version=EncodingVersion.V1):
        from framedb.version_store.helper import extract_lib_config
    ...
    def get_backing_store(self):
        ...
        try:
            ...
            storage_val = re.search("cxx.frame.org/framecc.pb2.(.*)_pb2.Config", primary_backing_url)
    ...
    def write(
        ...
        **kwargs,
        ...
        Note: FrameDB will use the 0-th level index of the Pandas DataFrame for its on-disk index.

        Any non-`DatetimeIndex` will converted into an internal `RowCount` index. That is, FrameDB will assign each
        ...
        validate_index: bool, default=False
            ...
            FrameDB relies on Pandas to detect if data is sorted - you can call DataFrame.index.is_monotonic_increasing on your input DataFrame to see if Pandas believes the
            data to be sorted
        kwargs :
    ...
    def append(
        ...
        **kwargs,
        ...
        validate_index: bool, default=False
            ...
            data and `data` are both sorted in ascending order. FrameDB relies on Pandas to detect if data is sorted - you can call DataFrame.index.is_monotonic_increasing
    ...
    def batch_write(
        ...
        **kwargs,
        ...
        validate_index: bool, default=False
            ...
            This in effect tests that the data is sorted in ascending order. FrameDB relies on Pandas to detect if data is sorted -
    ...
    @staticmethod
    def get_frame_style_type_info_for_norm(desc):
        ...
        # eg: https://github.com/manahl/frame/blob/master/frame/store/_pandas_ndarray_store.py#L177
    ...
    def _process_info(self, symbol: str, dit, as_of: Optional[VersionQueryInput] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "type": self.get_frame_style_type_info_for_norm(timeseries_descriptor),
            "date_range": date_range,
    ...
    def defragment_symbol_data(self, symbol: str, segment_size: Optional[int] = None) -> VersionedItem:
        """
        Compacts fragmented segments by merging row-sliced segments (https://docs.framedb.io/technical/on_disk_storage/#data-layer).
        ...
        https://docs.framedb.io/api/framedb/framedb.LibraryOptions
