...
from framecc.pb2.descriptors_pb2 import UserDefinedMetadata, NormalizationMetadata, MsgPackSerialization
from framecc.pb2.storage_pb2 import VersionStoreConfig
from mmap import mmap
from collections import Counter
from framedb.exceptions import ArcticNativeException, ArcticDbNotYetImplemented, NormalizationException, SortingException
from framedb.supported_types import DateRangeInput, time_types as supported_time_types
from framedb.util._versions import IS_PANDAS_TWO, IS_PANDAS_ZERO
from framedb.version_store.read_result import ReadResult
from framedb_ext.version_store import SortedValue as _SortedValue
...
from framedb import _msgpack_compat
from framedb.log import version
from framedb.version_store._common import _column_name_to_strings, TimeFrame
...
def _accept_array_string(v):
    # TODO remove this once frame keeps the string type under the hood
...
def _to_primitive(arr, arr_name, dynamic_strings, string_max_len=None, coerce_column_type=None, norm_meta=None):
    ...
    if "pyarrow" in arr_dtype_as_str:
        raise ArcticDbNotYetImplemented(
            "PyArrow-backed pandas DataFrame and Series are not currently supported by FrameDB. \n"
            "Please convert your pandas DataFrame and Series to use NumPy array before using them with FrameDB. \n"
            "If you are interested in the support of PyArrow-backed pandas DataFrame and Series, please upvote this \n"
            "GitHub issue and participate to its discussions: https://github.com/man-group/FrameDB/issues/881"
        )
    ...
    if np.issubdtype(arr.dtype, np.datetime64):
        # FrameDB only operates at nanosecond resolution (i.e. `datetime64[ns]`) type because so did Pandas < 2.
    ...
    def _index_to_records(self, df, pd_norm, dynamic_strings, string_max_len):
        ...
        if isinstance(index, MultiIndex):
            ...
            if IS_PANDAS_TWO and isinstance(index, RangeIndex) and n_rows == 0 and n_categorical_columns == 0:
                ...
                # Yet internally, FrameDB uses a DatetimeIndex for empty dataframes and series without categorical
                # columns.
                #
                # The index is converted to a DatetimeIndex for preserving the behavior of FrameDB with Pandas 1.0.
                index = DatetimeIndex([])
        ...
        if df.empty:
            ...
            for index_level_num in range(1, midx.field_count + 1):
                ...
                if not IS_PANDAS_TWO and index_col.dtype == "float":
                    ...
                    # See: https://github.com/man-group/FrameDB/pull/1049
...
def restrict_data_to_date_range_only(data: T, *, start: Timestamp, end: Timestamp) -> T:
    ...
    if hasattr(data, "loc"):
        ...
        if not data.index.is_monotonic_increasing:
            ...
            # See https://github.com/man-group/FrameDB/issues/1173 for more details
