...
from framecc.pb2.lmdb_storage_pb2 import Config as LmdbConfig
from framecc.pb2.s3_storage_pb2 import Config as S3Config
from framecc.pb2.azure_storage_pb2 import Config as AzureConfig
from framecc.pb2.in_memory_storage_pb2 import Config as MemoryConfig
from framecc.pb2.mongo_storage_pb2 import Config as MongoConfig
from framecc.pb2.nfs_backed_storage_pb2 import Config as NfsConfig
from framecc.pb2.storage_pb2 import (
...
from framedb.config import *  # for backward compat after moving to config
from framedb.config import _expand_path
from framedb.exceptions import ArcticNativeException, LibraryNotFound, UserInputException
...
def add_lmdb_library_to_env(cfg, lib_name, env_name, db_dir=Defaults.DATA_DIR, description=None, *, lmdb_config={}):
    ...
    storage.config.Pack(lmdb, type_url_prefix="cxx.frame.org")
...
def add_memory_library_to_env(cfg, lib_name, env_name, description=None):
    ...
    sid, storage = get_storage_for_lib_name(lib_name, env)
    storage.config.Pack(in_mem, type_url_prefix="cxx.frame.org")
    _add_lib_desc_to_env(env, lib_name, sid, description)
...
def get_mongo_proto(cfg, lib_name, env_name, uri):
    ...
    sid, storage = get_storage_for_lib_name(lib_name, env)
    storage.config.Pack(mongo, type_url_prefix="cxx.frame.org")
    return sid, storage
...
def get_s3_proto(
    ...
    storage.config.Pack(s3, type_url_prefix="cxx.frame.org")
    return sid, storage
...
def get_azure_proto(
    ...
    storage.config.Pack(azure, type_url_prefix="cxx.frame.org")
    return sid, storage
...
def get_frame_native_lib(lib_fqn):
