...
from framedb.adapters.prefixing_library_adapter_decorator import PrefixingLibraryAdapterDecorator

# All storage client libraries to be imported on-demand to speed up start-up of ad-hoc test runs
if TYPE_CHECKING:
    from pymongo import MongoClient

# Configure the root logger to INFO, since all FrameDB loggers default to INFO
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("Mongo Storage Fixture")
log_level = os.getenv("FRAMEDB_mongo_test_fixture_loglevel")
if log_level:
...
class MongoDatabase(StorageFixture):
    ...
    def __init__(self, mongo_uri: str, name: Optional[str] = None, client: Optional["MongoClient"] = None):
        ...
        self.frame_uri = PrefixingLibraryAdapterDecorator.prefix_uri(self.prefix, mongo_uri)

    def __exit__(self, exc_type, exc_value, traceback):
        self.libs_from_factory.clear()

        with handle_cleanup_exception(self, "prefix_mongo_database"):
            self.client.drop_database("framecc_" + self.prefix[:-1])
