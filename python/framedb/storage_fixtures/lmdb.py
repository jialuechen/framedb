...
from framedb.version_store.helper import add_lmdb_library_to_env
...
class LmdbStorageFixture(StorageFixture):
    def __init__(self, db_dir: Optional[str] = None):
        ...
        self.frame_uri = "lmdb://" + self.db_dir
        self.libs_instances_from_frame: List[NativeVersionStore] = []
        self.frame_instances: List[Arctic] = []

    def create_frame(self, **extras):
        out = super().create_frame(**extras)
        out._accessed_libs = self.libs_instances_from_frame
        self.frame_instances.append(out)
        return out

    def __exit__(self, exc_type, exc_value, traceback):
        for result in chain(self.libs_from_factory.values(), self.libs_instances_from_frame):
