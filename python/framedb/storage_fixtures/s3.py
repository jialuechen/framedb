...
from framedb.version_store.helper import add_s3_library_to_env
...
class S3Bucket(StorageFixture):
    ...
    def __init__(self, factory: "BaseS3StorageFixtureFactory", bucket: str):
        ...
        self.frame_uri = f"s3{secure or ''}://{host}:{self.bucket}?access={self.key.id}&secret={self.key.secret}"
        if port:
            self.frame_uri += f"&port={port}"
        if factory.default_prefix:
            self.frame_uri += f"&path_prefix={factory.default_prefix}"
...
def real_s3_from_environment_variables(*, shared_path: bool):
    ...
    out.endpoint = os.getenv("FRAMEDB_REAL_S3_ENDPOINT")
    out.region = os.getenv("FRAMEDB_REAL_S3_REGION")
    out.default_bucket = os.getenv("FRAMEDB_REAL_S3_BUCKET")
    ...
    out.clean_bucket_on_fixture_exit = os.getenv("FRAMEDB_REAL_S3_CLEAR").lower() in ["true", "1"]
    if shared_path:
        out.default_prefix = os.getenv("FRAMEDB_PERSISTENT_STORAGE_SHARED_PATH_PREFIX")
    else:
        out.default_prefix = os.getenv("FRAMEDB_PERSISTENT_STORAGE_UNIQUE_PATH_PREFIX")
    return out
