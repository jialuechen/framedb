...
from framedb.version_store.helper import ArcticMemoryConfig
from framedb.util.test import apply_lib_cfg
...
class StorageFixture(_SaferContextManager):
    ...
    def __init__(self):
        ...
        self.libs_names_from_frame: List[str] = []

    def __str__(self):
        return f"{type(self).__name__}[{self.frame_uri}]"

    def create_frame(self, **extras):
        """Similar to `Arctic(self.frame_uri)` but also keeps track of the libraries created"""
        out = Arctic(self.frame_uri, **extras)
        out._created_lib_names = self.libs_names_from_frame
        return out
    ...
    def slow_cleanup(self, failure_consequence=""):
        ...
        for lib in self.libs_names_from_frame[:]:
            with handle_cleanup_exception(self, lib, consequence=failure_consequence):
                frame.delete_library(lib)
