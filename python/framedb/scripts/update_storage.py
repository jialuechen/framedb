    ...
    if ac._library_manager.is_library_config_ok(lib_name, throw_on_failure=False):
        ...
        if run:
            ...
            try:
                ...
                logging.exception(
                    "Unexpected error, exiting. Not updating remaining libraries. Please report on FrameDB Github."
                )
                raise
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Upgrade FrameDB storage. This script checks for and performs (with "
            "`--run`) any data or config changes needed to support new versions of FrameDB."
        )
    )
    parser.add_argument(
        "--uri",
        required=True,
        help="URI for Arctic instance to fix, see https://docs.framedb.io/api/framedb#framedb.Arctic.__init__",
    )
    parser.add_argument(
        "--run",
        default=False,
