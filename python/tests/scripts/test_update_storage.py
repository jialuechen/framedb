...
from framedb.scripts.update_storage import run
...
from framedb.storage_fixtures.api import ArcticUriFields
from framedb.storage_fixtures.azure import AzureContainer
from framedb.storage_fixtures.s3 import S3Bucket
from framedb.adapters.s3_library_adapter import USE_AWS_CRED_PROVIDERS_TOKEN
...
def test_upgrade_script_dryrun_s3(s3_storage: S3Bucket):
    ...
    # When
    run(uri=s3_storage.frame_uri, run=False)
...
def test_upgrade_script_s3(s3_storage: S3Bucket):
    ...
    run(uri=s3_storage.frame_uri, run=True)
...
def test_upgrade_script_s3_rbac_ok(s3_storage: S3Bucket, monkeypatch):
    ...
    if os.name == "nt":
        ...
        from framedb_ext.tools import putenv_s
    ...
    uri = s3_storage.replace_uri_field(s3_storage.frame_uri, ArcticUriFields.USER, "aws_auth=true", start=1)
    uri = s3_storage.replace_uri_field(uri, ArcticUriFields.PASSWORD, "", start=1, end=3)
    s3_storage.frame_uri = uri

    ac = s3_storage.create_frame()  # create_frame() does proper clean up
...
def test_upgrade_script_dryrun_azure(azurite_storage: AzureContainer):
    # Given
    ac = azurite_storage.create_frame()
    create_library_config(ac, LIB_NAME)

    # When
    run(uri=azurite_storage.frame_uri, run=False)
...
def test_upgrade_script_azure(azurite_storage: AzureContainer):
    ...
    run(uri=azurite_storage.frame_uri, run=True)
