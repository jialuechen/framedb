...
def write_to_frame(df, symbol, version_store):
    start = time.time()
    vit = version_store.write(symbol, df)
    elapsed = time.time() - start
    print("frame write time: " + symbol + " " + str(elapsed))
    return vit
...
def test_stress(pct_null, repeats, symbol, object_version_store):
    ...
    write_to_frame(df, symbol, object_version_store)
    start = time.time()
    test_df = object_version_store.read(symbol).data
    elapsed = time.time() - start
    print("frame read time: " + str(elapsed))
...
def test_stress_small_row(pct_null, repeats, symbol, lmdb_or_in_memory_version_store_tiny_segment):
    ...
    print("Generated data, starting write")
    write_to_frame(df, symbol, lmdb_or_in_memory_version_store_tiny_segment)
    start = time.time()
