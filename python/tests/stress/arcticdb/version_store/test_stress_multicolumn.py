...
def test_stress_multicolumn(object_version_store):
    ...
    for count, test_data in enumerate(dataframes):
        ...
        print("saving to frame native: {}".format(pd.Timestamp("now") - now))

        now = pd.Timestamp("now")
        output_df = lib.read(name).data
        print("reading from frame native: {}".format(pd.Timestamp("now") - now))

        # FrameDB stores empty columns under a dedicated `EMPTYVAL` type, so the types are not going
