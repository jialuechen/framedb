...
from framedb.util.errors import *
import framedb.exceptions as ae
from framedb.exceptions import *  # keep as wildcard so all_exception_types below includes everything
from framedb_ext.exceptions import _ArcticLegacyCompatibilityException
...
def test_raise(code, exception):
    with pytest.raises(exception, match=f"E{code.value} a message$"):
        framedb_raise(code, lambda: "a message")
