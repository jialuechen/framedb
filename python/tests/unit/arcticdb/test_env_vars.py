...
from framedb.tools import set_config_from_env_vars, _FRAMEDB_ENV_VAR_PREFIX
...
def test_get_normal(key, value, mocks):
    set_config_from_env_vars({_FRAMEDB_ENV_VAR_PREFIX + key: str(value)})
...
def test_bad_format(key, value, mocks):
    with patch(_MODULE + ".logging") as log_mock:
        set_config_from_env_vars({_FRAMEDB_ENV_VAR_PREFIX + key: value})
