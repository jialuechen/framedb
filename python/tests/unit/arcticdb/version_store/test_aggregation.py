...
from framedb_ext.exceptions import InternalException, SchemaException
...
def test_hypothesis_mean_agg(lmdb_version_store, df):
    ...
    # FrameDB adopted this behaviour.
    #
...
def count_aggregation(lmdb_version_store, df):
    ...
    )  # New version of pandas treats values which exceeds limits as np.nan rather than np.inf, as in old version and framedb
