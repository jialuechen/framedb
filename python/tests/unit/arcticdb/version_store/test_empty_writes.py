...
def test_write_no_rows(lmdb_version_store, sym):
    ...
    # FrameDB stores empty columns under a dedicated `EMPTYVAL` type, so the types are not going to match with pandas
...
def test_update_no_columns_dynamic_schema(lmdb_version_store_dynamic_schema, sym):
    ...
    # update in frame native (outer join) behaves a bit differently than DataFrame.update (left join)
