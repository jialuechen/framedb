...
from framedb_ext.exceptions import InternalException, NormalizationException, SortingException
from framedb_ext import set_config_int
from framedb.util.test import random_integers, assert_frame_equal
from framedb.config import set_log_level
...
def test_read_incomplete_no_warning(s3_store_factory, sym, get_stderr):
    ...
    try:
        ...
        err = get_stderr()
        assert err.count("W framedb.storage | Failed to find segment for key") == 0
        assert err.count("D framedb.storage | Failed to find segment for key") == 1
    finally:
        set_log_level()
