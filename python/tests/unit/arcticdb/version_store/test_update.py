...
from framedb.util.test import random_strings_of_length, random_string, random_floats, assert_frame_equal
from framedb.exceptions import InternalException, SortingException
...
def test_non_cstyle_numpy_update(lmdb_version_store):
    ...
    after_frame = lmdb_version_store.read(symbol).data
    before_frame = pd.concat([sorted_df_1.iloc[:-1], sorted_df_2])
    assert_frame_equal(after_frame, before_frame)
