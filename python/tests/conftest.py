...
from framedb.storage_fixtures.api import StorageFixture
from framedb.storage_fixtures.azure import AzuriteStorageFixtureFactory
from framedb.storage_fixtures.lmdb import LmdbStorageFixture
from framedb.storage_fixtures.s3 import MotoS3StorageFixtureFactory, real_s3_from_environment_variables, mock_s3_with_error_simulation
from framedb.storage_fixtures.mongo import auto_detect_server
from framedb.storage_fixtures.in_memory import InMemoryStorageFixture
from framedb.version_store._normalization import MsgPackNormalizer
from framedb.util.test import create_df
...
def get_stderr(capfd):
    from framedb_ext.log import flush_all
...
def frame_client(request, encoding_version):
    storage_fixture: StorageFixture = request.getfixturevalue(request.param + "_storage")
    ac = storage_fixture.create_frame(encoding_version=encoding_version)
...
def frame_client_no_lmdb(request, encoding_version):
    storage_fixture: StorageFixture = request.getfixturevalue(request.param + "_storage")
    ac = storage_fixture.create_frame(encoding_version=encoding_version)
    assert not ac.list_libraries()
    return ac
...
@pytest.fixture
def frame_library(frame_client, lib_name):
    return frame_client.create_library(lib_name)
