...
from framedb.exceptions import LmdbMapFullError, StorageException
from framedb.util.test import get_wide_dataframe
import framedb.adapters.lmdb_library_adapter as la
from framedb.exceptions import LmdbOptionsError


def test_batch_read_only_segfault_regression(lmdb_storage):
    # See Github issue #520
    # This segfaults with framedb==1.5.0
    ac = lmdb_storage.create_frame()
    ...
    fresh_lib = lmdb_storage.create_frame()["test_lib"]
...
def create_frame_instance(td, i):
    ac = Arctic(f"lmdb://{td}")
    lib = ac["test"]
    assert lib.read("a")
...
def test_warnings_frame_instance(tmp_path, get_stderr):
...
def test_frame_instances_across_same_lmdb_multiprocessing(tmp_path):
    """Should not warn when across multiple processes."""
    ac = Arctic(f"lmdb://{tmp_path}")
    ac.create_library("test")
    ac["test"].write("a", pd.DataFrame())
    with mp.Pool(5) as p:
        p.starmap(create_frame_instance, [(tmp_path, i) for i in range(20)])
