...
from framedb.version_store import VersionedItem as PythonVersionedItem
from framedb_ext.storage import KeyType
from framedb_ext.version_store import VersionRequestType

from framedb.options import LibraryOptions
from framedb import QueryBuilder, DataError
from framedb_ext.version_store import AtomKey, RefKey
...
import numpy as np
from framedb.util.test import (
    assert_frame_equal,
    distinct_timestamps,
    random_strings_of_length,
    random_floats,
)
from framedb.util._versions import IS_PANDAS_TWO
from framedb_ext import set_config_int
...
@pytest.fixture
def library_factory(frame_client, lib_name):
    def create_library(library_options=None, name: str = lib_name):
        return frame_client.create_library(name, library_options)
...
def test_write_meta_batch_with_as_ofs(frame_library):
    lib = frame_library
    num_symbols = 2
    num_versions = 5
...
def test_write_metadata_batch_with_none(frame_library):
    lib = frame_library
    symbol = "symbol_"
    num_symbols = 2
...
def test_read_meta_batch_with_as_ofs(frame_library):
    lib = frame_library
...
def test_read_metadata_batch_with_none(frame_library):
    lib = frame_library
...
def test_read_metadata_batch_missing_keys(frame_library):
    lib = frame_library
...
def test_read_metadata_batch_symbol_doesnt_exist(frame_library):
    lib = frame_library
...
def test_read_metadata_batch_version_doesnt_exist(frame_library):
    lib = frame_library
...
def test_write_batch_with_pickle_mode(frame_library):
...
def test_write_object_in_batch_without_pickle_mode(frame_library):
    ...
    # omit the part with the full class path as that will change in framedb
...
def test_write_object_in_batch_without_pickle_mode_many_symbols(frame_library):
    """Writing outside of pickle mode should fail when the user does not use the dedicated method."""
    lib = frame_library
    with pytest.raises(ArcticUnsupportedDataTypeException) as e:
...
def test_write_batch_duplicate_symbols(frame_library):
...
def test_write_pickle_batch_duplicate_symbols(frame_library):
    """Should throw and not write if duplicate symbols are provided."""
    lib = frame_library
    with pytest.raises(ArcticDuplicateSymbolsInBatchException):
        lib.write_pickle_batch(
...
def test_append_batch_missing_keys(frame_library):
    lib = frame_library
...
def test_read_batch_time_stamp(frame_library):
...
def test_read_batch_mixed_request_supported(frame_library):
    lib = frame_library
...
def test_read_batch_with_columns(frame_library):
    lib = frame_library
...
def test_read_batch_overall_query_builder(frame_library):
    lib = frame_library
...
def test_read_batch_per_symbol_query_builder(frame_library):
    lib = frame_library
...
def test_read_batch_as_of(frame_library):
    lib = frame_library
...
def test_batch_methods_with_negative_as_of(frame_library):
    lib = frame_library
...
def test_read_batch_date_ranges(frame_library):
    lib = frame_library
...
def test_read_batch_date_ranges_dates_not_times(frame_library):
    lib = frame_library
    df = pd.DataFrame({"column": [1, 2, 3, 4]}, index=pd.date_range(start="1/1/2018", end="1/4/2018"))
    lib.write("symbol", df)
...
def test_read_batch_overall_query_builder_and_per_request_query_builder_raises(frame_library):
    lib = frame_library
...
def test_read_batch_unhandled_type(frame_library):
...
def test_read_batch_symbol_doesnt_exist(frame_library):
    lib = frame_library
...
def test_read_batch_version_doesnt_exist(frame_library):
    lib = frame_library
...
def test_read_batch_missing_keys(frame_library):
    lib = frame_library
...
def test_write_metadata_batch_missing_keys(frame_library):
    lib = frame_library
...
def test_read_batch_query_builder_missing_keys(frame_library):
    lib = frame_library
...
def test_get_description_batch_missing_keys(frame_library):
    lib = frame_library
...
def test_get_description_batch_symbol_doesnt_exist(frame_library):
    lib = frame_library
...
def test_get_description_batch_version_doesnt_exist(frame_library):
    lib = frame_library
...
def test_read_batch_query_builder_symbol_doesnt_exist(frame_library):
    lib = frame_library
...
def test_read_batch_query_builder_version_doesnt_exist(frame_library):
    lib = frame_library
...
def test_get_description_batch(frame_library):
    lib = frame_library
...
def test_get_description_batch_multiple_versions(frame_library):
    lib = frame_library
...
def test_read_description_batch_high_amount(frame_library):
    lib = frame_library
    num_symbols = 10
    num_versions = 4
    start_year = 2000
...
def test_read_description_batch_empty_nat(frame_library):
    lib = frame_library
    num_symbols = 10
    for sym in range(num_symbols):
...
def test_read_batch_mixed_with_snapshots(frame_library):
    num_symbols = 10
    num_versions = 10
