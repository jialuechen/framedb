...
from framedb.exceptions import (
...
from framedb.version_store import NativeVersionStore
from framedb.version_store._custom_normalizers import CustomNormalizer, register_normalizer
from framedb.version_store._store import VersionedItem
from framedb_ext.exceptions import _ArcticLegacyCompatibilityException, StorageException
from framedb_ext.storage import KeyType, NoDataFoundException
from framedb_ext.version_store import NoSuchVersionException, StreamDescriptorMismatch, ManualClockVersionStore
from framecc.pb2.descriptors_pb2 import NormalizationMetadata  # Importing from framedb dynamically loads framecc.pb2
...
def test_negative_cases(basic_store, symbol):
    df = sample_dataframe()
    # To stay consistent with frame this doesn't throw.
    basic_store.delete("does_not_exist")
...
def test_coercion_to_str_with_dynamic_strings(basic_store):
    ...
    with mock.patch(
        "framedb.version_store._normalization.get_sample_from_non_empty_arr", return_value="hello"
    ) as sample_mock:
        # This should succeed but uses the slow path
        lib.write("sym", df, dynamic_strings=True)
        sample_mock.assert_called()

    with mock.patch("framedb.version_store._normalization.get_sample_from_non_empty_arr") as sample_mock:
...
def test_restore_version(basic_store_tiny_segment):
    lib = basic_store_tiny_segment
    # Triggers bug https://github.com/man-group/FrameDB/issues/469 by freezing time
