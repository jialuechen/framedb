...
from framedb_ext.exceptions import InternalException, UserInputException
from framedb_ext.storage import NoDataFoundException
from framedb.exceptions import ArcticDbNotYetImplemented, LibraryNotFound, MismatchingLibraryOptions
...
from framedb import QueryBuilder
from framecc.pb2.s3_storage_pb2 import Config as S3Config
from framedb.storage_fixtures.api import StorageFixture, ArcticUriFields, StorageFixtureFactory
from framedb.storage_fixtures.mongo import MongoDatabase
from framedb.util.test import assert_frame_equal
from framedb.storage_fixtures.s3 import S3Bucket
from framedb.version_store.library import (
    WritePayload,
...
def test_library_creation_deletion(frame_client):
    ac = frame_client
    ...
    if "mongo" in frame_client.get_uri():
...
def test_get_library(frame_client):
    ac = frame_client
    assert ac.list_libraries() == []
...
def test_create_library_with_invalid_name(frame_client):
    ac = frame_client
...
# TODO: Fix issue #1247, then use "arcitc_client" instead of "frame_client_no_lmdb"
@pytest.mark.parametrize("prefix", ["", "prefix"])
@pytest.mark.parametrize("suffix", ["", "suffix"])
def test_create_library_with_all_chars(frame_client_no_lmdb, prefix, suffix):
...
def test_do_not_persist_s3_details(s3_storage):
    ...
    ac = Arctic(s3_storage.frame_uri)
...
def test_library_options(frame_client):
    ac = frame_client
    assert ac.list_libraries() == []
...
def test_separation_between_libraries(frame_client):
...
def test_separation_between_libraries_with_prefixes(fixture, request):
    ...
    mercury_uri = add_path_prefix(storage_fixture.frame_uri, "/planet/mercury")
    ac_mercury = Arctic(mercury_uri)

    mars_uri = add_path_prefix(storage_fixture.frame_uri, "/planet/mars")
...
def test_library_management_path_prefix(fixture, request):
    storage_fixture: StorageFixture = request.getfixturevalue(fixture)
    uri = add_path_prefix(storage_fixture.frame_uri, "hello/world")
    ac = Arctic(uri)
    assert ac.list_libraries() == []
    ...
    assert any(k.startswith("hello/world/_frame_cfg") for k in keys)
...
def test_basic_write_read_update_and_append(frame_library):
    lib = frame_library
...
def test_write_metadata_with_none(frame_library):
    lib = frame_library
    symbol = "symbol"
...
def staged_write(sym, frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    df.index = pd.date_range("2018-01-01", periods=3, freq="H")
...
def test_parallel_writes_and_appends(frame_library):
    lib = frame_library
...
def test_snapshots_and_deletes(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("my_symbol", df)
...
def test_delete_non_existent_snapshot(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("my_symbol", df)
...
def test_prune_previous_versions(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
...
def test_do_not_prune_previous_versions_by_default(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("symbol", df)
    lib.write("symbol", df)
...
def test_delete_version(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("symbol", df, metadata={"very": "interesting"})
    lib.write("symbol", df, metadata={"muy": "interesante"}, prune_previous_versions=False)
...
def test_list_versions_write_append_update(frame_library):
    lib = frame_library
...
def test_list_versions_latest_only(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("symbol", df)
    lib.write("symbol", df)
...
def test_non_existent_list_versions_latest_only(frame_library):
    lib = frame_library
...
def test_delete_version_with_snapshot(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("symbol", df, metadata={"very": "interesting"})
    lib.write("symbol", df, metadata={"muy": "interesante"}, prune_previous_versions=False)
...
def test_list_versions_with_snapshot(frame_library):
    lib = frame_library
    df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
    lib.write("symbol", df, metadata={"very": "interesting"})
    lib.write("symbol", df, metadata={"muy": "interesante"}, prune_previous_versions=False)
...
def test_list_versions_without_snapshot(frame_library):
    lib = frame_library
...
def test_delete_version_that_does_not_exist(frame_library):
    lib = frame_library
...
def test_delete_date_range(frame_library):
    lib = frame_library
...
def _test_mongo_repr_body(mongo_storage: MongoDatabase):
    # The frame_uri has the PrefixingLibraryAdapterDecorator logic in it, so use mongo_uri
...
def test_mongo_construction_no_pymongo(monkeypatch, mongo_storage: MongoDatabase):
    import framedb.adapters.mongo_library_adapter

    monkeypatch.setattr(framedb.adapters.mongo_library_adapter, "_HAVE_PYMONGO", False)

    _test_mongo_repr_body(mongo_storage)


def test_s3_repr(s3_storage: S3Bucket, one_col_df):
    ac = s3_storage.create_frame()
    assert ac.list_libraries() == []
...
def test_write_object_with_pickle_mode(frame_library):
...
def test_write_object_without_pickle_mode(frame_library):
...
def test_write_list_without_pickle_mode(frame_library):
    """Writing outside of pickle mode should fail when the user does not use the dedicated method."""
    lib = frame_library
    with pytest.raises(ArcticUnsupportedDataTypeException):
...
def test_write_non_native_frame_with_pickle_mode(frame_library):
...
def test_write_non_native_frame_without_pickle_mode(frame_library):
    """Writing outside of pickle mode should fail when the user does not use the dedicated method."""
    lib = frame_library
    df = pd.DataFrame({"col1": [A("id_1")]})
...
def test_write_with_unpacking(frame_library):
...
def test_prune_previous_versions_with_write(frame_library):
    lib = frame_library
    # When
...
def test_append_documented_example(frame_library):
    lib = frame_library
...
def test_append_prune_previous_versions(frame_library):
    lib = frame_library
    df = pd.DataFrame({"column": [1, 2, 3]}, index=pd.date_range(start="1/1/2018", end="1/3/2018"))
    lib.write("symbol", df)
    to_append_df = pd.DataFrame({"column": [4, 5, 6]}, index=pd.date_range(start="1/4/2018", end="1/6/2018"))
...
def test_update_documented_example(frame_library):
...
def test_update_prune_previous_versions(frame_library):
...
def test_update_with_daterange(frame_library):
    lib = frame_library
    df = pd.DataFrame({"column": [1, 2, 3, 4]}, index=pd.date_range(start="1/1/2018", end="1/4/2018"))
    lib.write("symbol", df)
...
def test_update_with_daterange_no_width(frame_library):
    lib = frame_library
    df = pd.DataFrame({"column": [1, 2, 3, 4]}, index=pd.date_range(start="1/1/2018", end="1/4/2018"))
    lib.write("symbol", df)
...
def test_update_with_daterange_multi_index(frame_library):
    lib = frame_library
    # Given
...
def test_update_with_daterange_multi_index_no_width(frame_library):
    lib = frame_library
...
def test_update_with_daterange_restrictive(frame_library):
...
def test_update_with_upsert(frame_library):
    lib = frame_library
    with pytest.raises(Exception):
...
def test_read_with_read_request_form(frame_library):
    lib = frame_library
...
def test_has_symbol(frame_library):
    lib = frame_library
    lib.write("symbol", pd.DataFrame())
...
def test_numpy_string(frame_library):
    frame_library.write("symbol", np.array(["ab", "cd", "efg"]))
    res = frame_library.read("symbol").data
    np.testing.assert_array_equal(res, np.array(["ab", "cd", "efg"]))


@pytest.mark.skipif(sys.platform != "win32", reason="SKIP_WIN Numpy strings not supported yet")
def test_numpy_string_fails_on_windows(frame_library):
    with pytest.raises(ArcticDbNotYetImplemented):
        frame_library.write("symbol", np.array(["ab", "cd", "efg"]))


def test_get_description(frame_library):
    lib = frame_library
...
def test_segment_slicing(frame_client):
    ac = frame_client
    assert ac.list_libraries() == []
    rows_per_segment = 5
...
def test_reload_symbol_list(fixture, request):
    ...
    ac = Arctic(storage_fixture.frame_uri)
    assert ac.list_libraries() == []
...
def test_get_uri(fixture, request):
    storage_fixture: StorageFixture = request.getfixturevalue(fixture)
    ac = storage_fixture.create_frame()
    assert ac.get_uri() == storage_fixture.frame_uri


@AZURE_TESTS_MARK  # GH issue #1060
def test_azure_no_ca_path(azurite_storage: StorageFixture):
    uri = azurite_storage.replace_uri_field(azurite_storage.frame_uri, ArcticUriFields.CA_PATH, "", start=1, end=3)
...
def test_azure_sas_token(azurite_storage_factory: StorageFixtureFactory):
    with azurite_storage_factory.enforcing_permissions_context():
        ...
        with azurite_storage_factory.create_fixture() as f:
            assert "SharedAccessSignature" in f.frame_uri
...
def test_s3_force_uri_lib_config_handling(s3_storage):
    ...
    # Why it becomes obsolete: https://github.com/man-group/FrameDB/pull/803
    Arctic(s3_storage.frame_uri + "&force_uri_lib_config=true")

    with pytest.raises(ValueError):
        Arctic(s3_storage.frame_uri + "&force_uri_lib_config=false")
