...
from framedb.version_store.library import WritePayload, ReadRequest
...
def shared_persistent_frame_client(real_s3_storage_without_clean_up, encoding_version):
    return real_s3_storage_without_clean_up.create_frame(encoding_version=encoding_version)


# TODO: Add a check if the real storage tests are enabled
@pytest.mark.parametrize("library", LIBRARIES)
@REAL_S3_TESTS_MARK
def test_real_s3_storage_read(shared_persistent_frame_client, library):
    ac = shared_persistent_frame_client
    lib = ac[library]
    read_persistent_library(lib)


@REAL_S3_TESTS_MARK
def test_real_s3_storage_write(shared_persistent_frame_client):
    strategy_branch = os.getenv("FRAMEDB_PERSISTENT_STORAGE_STRATEGY_BRANCH")
...
@pytest.fixture(params=[pytest.param("REAL_S3", marks=REAL_S3_TESTS_MARK)])
def persistent_frame_client(real_s3_storage, encoding_version):
    return real_s3_storage.create_frame(encoding_version=encoding_version)


@pytest.mark.parametrize("num_rows", [1_000_000])
def test_persistent_storage_read_write_large_data_ascending(persistent_frame_client, num_rows):
    ac = persistent_frame_client
...
def test_persistent_storage_read_write_large_data_random(persistent_frame_client, num_rows):
    ac = persistent_frame_client
...
def test_persistent_storage_read_write_many_syms(persistent_frame_client, num_syms, three_col_df):
