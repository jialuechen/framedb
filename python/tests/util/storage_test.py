...
# contains the create_df function from the framedb.util.test library
...
def real_s3_credentials(shared_path: bool = True):
    endpoint = os.getenv("FRAMEDB_REAL_S3_ENDPOINT")
    bucket = os.getenv("FRAMEDB_REAL_S3_BUCKET")
    region = os.getenv("FRAMEDB_REAL_S3_REGION")
    access_key = os.getenv("FRAMEDB_REAL_S3_ACCESS_KEY")
    secret_key = os.getenv("FRAMEDB_REAL_S3_SECRET_KEY")
    if shared_path:
        path_prefix = os.getenv("FRAMEDB_PERSISTENT_STORAGE_SHARED_PATH_PREFIX")
    else:
        path_prefix = os.getenv("FRAMEDB_PERSISTENT_STORAGE_UNIQUE_PATH_PREFIX")

    clear = str(os.getenv("FRAMEDB_REAL_S3_CLEAR")).lower() in ("true", "1")
...
def get_real_s3_uri(shared_path: bool = True):
    ...
    # https://github.com/man-group/FrameDB/blob/master/.github/workflows/persistent_storage.yml#L64
    (
        endpoint,
        bucket,
        region,
...
def read_persistent_library(lib):
    ...
    df, sym = get_csv_df()
    res_df = lib.read(sym).data
    # 'cast' to str to accomodate previous versions of FrameDB
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "The version of FrameDB that is used for seed, this is to make the name of the library that is seeded"
            " unique"
        ),
    )
